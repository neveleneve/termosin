                                    ...
                                    @foreach ($datakeranjang as $item)
                                        <tr class="text-nowrap">
                                            ...
                                            <td>
                                                ...
                                                <input type="hidden" id="harga{{ $item->id }}"
                                                    value="{{ $item->harga }}">
                                            </td>
                                            <td>
                                                {{ $dataitem[1]->status }}
                                                {{-- @if ($dataitem->where('id_item', $item->id_item)->status == 1)
                                                    <input class="form-control-plaintext" id="jumlah{{ $item->id }}"
                                                        name="jumlah[]" oninput="jumlahbelanja({{ $item->id }})"
                                                        type="number" value="{{ $item->jumlah }}" min="1">
                                                @else
                                            ...
                                            <td>
                                                <span class="float-right" name="subtotal{{ $item->id }}"
                                                    id="subtotal{{ $item->id }}">
                                                    Rp {{ number_format($subtotal, 2, ',', '.') }}
                                                </span>
                                                <input class="subtotal" type="hidden"
                                                    id="hiddensubtotal{{ $item->id }}" value="{{ $subtotal }}">
                                                @php
                                                    $total += $subtotal;
                                                @endphp
                                            </td>
